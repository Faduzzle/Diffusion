...
from tqdm import tqdm  # ✅ Use this for terminal scripts
...
def predict_and_save_inline(checkpoint_path, history_len=50, predict_len=50, input_dim=1,
                            num_samples=100, num_diffusion_timesteps=1000):
    ...
    # Generate full sine wave: history + future
    ...
    # ✅ Set save path relative to script location
    ...
    with torch.no_grad():
        for i in tqdm(range(num_diffusion_timesteps - 1, -1, -1), desc="Sampling", ncols=100):
    ...
    print(f"✅ Saved predictions and plots to: {save_dir}")
